...
@router.post("/execute")
def execute_action(req: ExecuteRequest, db: Session = Depends(get_db)):
    init_login_time()
    req.avg_amount = USER_HISTORY["avg_transaction"]
    current_hour = req.hour_of_day if req.hour_of_day is not None else datetime.datetime.now().hour
    is_round = req.is_round_number if req.is_round_number is not None else (req.amount % 10000 == 0)
    ...
    if decision == "BLOCK":
        ...
        response.update({
            "status": "blocked",
            "message": f"Action temporarily blocked. Try again after {cd} seconds.",
            "next_step": "Please contact your bank or try again after 24 hours.",
            "cooldown": cd,
            "nudge": generate_ai_nudge(req, level),
            "recommendation": f"Try investing ₹10,000 first or wait {cd*2} seconds before retrying."
        })
    elif decision == "WARN":
        cd = 5
        response.update({
            "status": "delayed",
            "message": f"Please wait {cd} seconds before proceeding.",
            "cooldown": cd,
            "nudge": generate_ai_nudge(req, level),
            "simulation_preview": generate_simulation(monthly, years, req.expected_return),
            "recommendation": "Proceed with a smaller amount or confirm after the cooldown."
        })
    else:
        ...
        response.update({
            ...
            "nudge": generate_ai_nudge(req, level),
            "simulation": generate_simulation(monthly, years, req.expected_return)
        })
...
def get_execution_audit(db: Session = Depends(get_db)):
    ...
    return {"total_evaluations": len(audit_log), "log": audit_log[-20:]}

@router.post("/simulate/fraud")
def simulate_fraud(db: Session = Depends(get_db)):
    req = ExecuteRequest(
